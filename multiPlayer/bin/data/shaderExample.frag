...
void main()
{
	...
	*/
    ...
    mediump float colorR1 = texture2D( tex0, texcoord0 ).r;
    mediump float colorG1 = texture2D( tex0, texcoord0 ).g;
    mediump float colorB1 = texture2D( tex0, texcoord0 ).b;  
	
	mediump float colorR2 = texture2D( tex1, texcoord0 ).r;
    mediump float colorG2 = texture2D( tex1, texcoord0 ).g;
    mediump float colorB2 = texture2D( tex1, texcoord0 ).b;  
	
    mediump float colorR = colorR1+ ( colorR2 - colorR1 ) * alpha ;
    mediump float colorG = colorG1+ ( colorG2 - colorG1 ) * alpha ;
    mediump float colorB = colorB1+ ( colorB2 - colorB1 ) * alpha ;  
	mediump float colorA = 1.0;     
